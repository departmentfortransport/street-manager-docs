...
![sequence diagram](images/permit-lifecycle.png)
...
Retrieves a list of sites that have been added to any works record associate with the currently authenticated user's organization. Sites are issued via the street manager API (see Sites and reinstatement section). Similarly to the permits endpoint, sites can be filtered by status. The status of a site are:
{: .govuk-body}

<ol class="govuk-list govuk-list--bullet">
  <li><strong>interim</strong></li>
